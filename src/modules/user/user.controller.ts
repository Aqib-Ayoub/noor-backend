...
export const getMasjidContext = async (req: Request, res: Response): Promise<void> => {
  ...
  // ── No Masjid linked ──
  if (!user.masjidRef) {
    // Check if user has a pending join request to any masjid
  ...
  const masjid = await Masjid.findById(user.masjidRef)
    .populate('adminRef', 'name phone');   // ← need admin's phone for contact default
  if (!masjid) {
  ...
  sendSuccess(
    res,
    {
      ...
      masjid: {
        ...
        /** Primary admin's phone — shown as default contact in settings */
